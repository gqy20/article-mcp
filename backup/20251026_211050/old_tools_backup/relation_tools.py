...
# 在注册时会注入这些依赖
relation_tools_deps = {"literature_relation_service": None, "logger": None}
...
def register_relation_tools(mcp, literature_relation_service, logger):
    ...
    def get_similar_articles(
        identifier: str, id_type: str = "doi", email: str | None = None, max_results: int = 20
    ) -> dict[str, Any]:
        ...
        try:
            ...
            result = literature_relation_service.get_similar_articles(
                identifier.strip(), id_type=id_type.lower(), max_results=max_results
            )
    ...
    def get_citing_articles(
        identifier: str, id_type: str = "pmid", max_results: int = 20, email: str | None = None
    ) -> dict[str, Any]:
        """获取引用该文献的文献信息
        ...
        try:
            if not identifier or not identifier.strip():
                return {"citing_articles": [], "total_count": 0, "error": "文献标识符不能为空"}
            ...
            return {"citing_articles": [], "total_count": 0, "error": str(e)}

    @mcp.tool()
    def get_literature_relations(
        identifier: str, id_type: str = "doi", max_results: int = 20
    ) -> dict[str, Any]:
