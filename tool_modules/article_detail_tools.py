...
# 在注册时会注入这些依赖
article_detail_tools_deps = {"europe_pmc_service": None, "logger": None}
...
def register_article_detail_tools(mcp, europe_pmc_service, logger):
    ...
    def get_article_details(
        identifier: str, id_type: str = "pmid", mode: str = "sync", include_fulltext: bool = False
    ) -> dict[str, Any]:
        """获取特定文献的详细信息（高性能优化版本）
        ...
        # 调用EuropePMC服务获取文献详情
        result = europe_pmc_service.fetch(
            identifier, id_type=id_type, mode=mode, include_fulltext=include_fulltext
        )
