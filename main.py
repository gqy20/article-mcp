...
def create_mcp_server():
    ...
    @mcp.tool()
    def get_article_details(identifier: str, id_type: str = "pmid", mode: str = "sync") -> Dict[str, Any]:
        """获取特定文献的详细信息（高性能优化版本）
        ...
        """
        # 调用EuropePMC服务获取文献详情
        result = europe_pmc_service.fetch(identifier, id_type=id_type, mode=mode)
    ...
    def get_citing_articles(
        ...
        - identifier: 必需，文献标识符
        - id_type: 可选，标识符类型，"pmid"（默认）、"doi" 或 "pmcid"
        - max_results: 可选，返回的最大引用文章数量，默认20篇
        - email: 可选，联系邮箱，用于获得更高的API访问限制
        
        返回值说明：
        - citing_articles: 引用文献列表
        - total_count: 总引用文献数量
        - message: 处理信息
        - error: 错误信息（如果有）
        - processing_time: 处理耗时（秒）
