...
def merge_articles_by_doi(articles_by_source: Dict[str, List[Dict]]) -> List[Dict]:
    ...
    for source, articles in articles_by_source.items():
        for article in articles:
            if not article.get('doi') and article not in [a for merged in merged_articles for a in merged.get('sources', [])]:
...
def extract_identifier_type(identifier: str) -> str:
    ...
    # arXiv ID检测
    if identifier.startswith('arXiv:') or ('.' in identifier and identifier.replace('.', '').replace('-', '').replace('_', '').isalnum()):
        return 'arxiv_id'
