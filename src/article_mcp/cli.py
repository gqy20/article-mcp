...
def start_server(
    ...
    safe_print("")
    safe_print("[工具6] export_batch_results")
    safe_print("   - 通用结果导出工具")
    safe_print("   - 支持: JSON、CSV、Excel等格式导出")
    safe_print("   - 特点: 多格式支持、元数据包含、自动路径生成")
    safe_print("   - 参数: results, format_type, output_path, include_metadata")
    safe_print("")
    safe_print("[技术特性]:")
