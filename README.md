...
- 🚀 **并发性能**: 支持真正的并发API调用，多源搜索速度提升 **3-5倍**
- 📊 **吞吐量**: 同时查询多个数据库，不再受同步阻塞限制
- 🔧 **AsyncAPIClient**: 统一异步HTTP客户端，支持连接池、自动重试、超时控制
- 🏗️ **FastMCP v2**: 升级到最新版本，更好的异步支持和错误处理
...
┌─────────────────────────────────────────────────────────┐
│                    AI 客户端                              │
│         (Claude Desktop, Cherry Studio, etc.)           │
└──────────────────────┬──────────────────────────────────┘
                       │ MCP Protocol
                       ▼
┌─────────────────────────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────┤
│  6个核心工具 (全部异步)                                  │
│  • search_literature (多源搜索)                         │
│  • get_article_details (文献详情)                       │
│  • get_references (参考文献)                            │
│  • get_literature_relations (关系分析)                  │
│  • get_journal_quality (期刊质量)                      │
│  • export_batch_results (批量导出)                      │
└──────────────────────┬──────────────────────────────────┘
                       │ async def
                       ▼
┌─────────────────────────────────────────────────────────┐
│                  服务层 (异步)                            │
├─────────────────────────────────────────────────────────┤
│  Europe PMC │ PubMed │ ArXiv │ CrossRef │ OpenAlex    │
│  search_async()  search_async()  search_async()  ...    │
└──────────────────────┬──────────────────────────────────┘
                       │ aiohttp
                       ▼
┌─────────────────────────────────────────────────────────┐
│               AsyncAPIClient (统一HTTP客户端)              │
│  • 连接池复用  • 自动重试  • 超时控制  • 并发限制          │
└─────────────────────────────────────────────────────────┘
```
...
| 单源搜索 | 1.5s | 1.2s | 20% |
| 三源并行 | 4.5s | 1.5s | **3x** |
| 五源并行 | 7.5s | 2.0s | **3.75x** |
| 批量查询(10) | 15s | 3s | **5x** |

> 注：实际性能取决于网络条件和API速率限制

### 向后兼容性

✅ **完全兼容** - MCP 工具接口保持不变，无需修改客户端配置
